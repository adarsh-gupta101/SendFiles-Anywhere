...
document.getElementById('upload-form').addEventListener('submit', (event) => {
  ...
  if (file) {
    ...
    reader.onload = (event) => {
      socket.emit('file-upload', { name: file.name, data: event.target.result,room:roomss });
    };
...
socket.on('file-received', (file) => {
 ...
 if (isImage(file.name)) {
    // Display the image
    fileData.innerHTML = `<img src="data:image/*;base64,${arrayBufferToBase64(file.data)}" />`;
 }
  else if (isVideo(file.name)) {
    // Display the video
    fileData.innerHTML = `<video controls width="400" height="300">
      <source src="data:video/mp4;base64,${arrayBufferToBase64(file.data)}" type="video/mp4" />
